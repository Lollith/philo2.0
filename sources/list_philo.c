...
t_philo	*ft_lstnew(int content, t_rules *rules)
{
	...
	new_elem->state = (int*) ft_calloc(3, sizeof (int) ); // initialisatioon lors de la creation des maillons, + simples que initialisation apres
	new_elem->nb_meal = rules->nb_t_must_eat; // penser a initialiser mes donne apres la creation de la sturcture en .h de m liste.:wq
	new_elem->m_state = (pthread_mutex_t*) ft_calloc(3, sizeof(pthread_mutex_t));
	if (!new_elem->m_state)
		return (NULL); //0
	int i = 0;
